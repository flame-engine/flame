...
void main() {
  group('Quaternion extensions', () {
    test('can lerp', () {
      ...
      final result = QuaternionExtension.lerp(a, b, 0.5);
      expect(result.x, 2);
      expect(result.y, 3);
      expect(result.z, 4);
      expect(result.w, 5);
    ...
    test('can slerp', () {
      ...
      final slerp1 = QuaternionExtension.slerp(quaternion1, quaternion2, 0.5);
      expect(
        angle1 - 0.5 * (angle1 - angle2),
        closeTo(slerp1.radians, _epsilon),
      );

      final slerp2 = QuaternionExtension.slerp(quaternion2, quaternion3, 0.75);
      expect(
        angle2 + 0.75 * (angle3 - angle2),
        closeTo(slerp2.radians, _epsilon),
      );

      final slerp3 = QuaternionExtension.slerp(quaternion2, quaternion2, 0.5);
