...
class TexturePackerAtlas {
  ...
  static Future<TexturePackerAtlas> load(
    ...
    final atlasData =
        await loadAtlas(path, fromStorage: fromStorage, images: images);

    return TexturePackerAtlas.fromAtlas(atlasData,
        whiteList: whiteList, useOriginalSize: useOriginalSize);
  }
  ...
  static Future<TextureAtlasData> loadAtlas(
    ...
    if (fromStorage) {
      ...
      return _fromAssets(path, images: images);
    }
  }
...
Future<TextureAtlasData> _fromAssets(String path, {Images? images}) async {
  try {
...
/// Uses the [path] to find the image directory.
Future<TextureAtlasData> _fromStorage(String path, {Images? images}) async {
  try {
