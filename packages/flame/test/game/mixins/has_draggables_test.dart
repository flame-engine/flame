...
class _DraggablesComponent extends PositionComponent with Draggables {
  _DraggablesComponent() : super(size: Vector2.all(100));
...
void main() {
  ...
  group('HasDraggables', () {
    testWithGame<_GameWithDraggables>(
      ...
      (game) async {
        await game.ensureAdd(_DraggablesComponent());
      },
    );
    ...
    testWithFlameGame(
      ...
      (game) async {
        expect(
          () => game.ensureAdd(_DraggablesComponent()),
          failsAssert(
            'Draggables Components can only be added to a FlameGame with '
