...
const int samples = 60; // The number of "copies" of the canvas made to emulate the "smoke" effect
...
vec4 occlusion(vec2 uv, vec2 lightpos, vec4 objects) {
    float lum = dot(objects.rgb, vec3(0.2126, 0.7152, 0.0722));
    objects.a = objects.a * 0.2 + lum * objects.a;
