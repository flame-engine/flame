...
class DraggablesExample extends FlameGame with HasDraggables {
  static const String description = '''
    In this example we show you can use the `Draggables` mixin on
    `PositionComponent`s. Drag around the Embers and see their position
    changing.
  ''';

  final double zoom;
  late final DraggablesEmber square;
  ...
  Future<void> onLoad() async {
    camera.zoom = zoom;
    add(square = DraggablesEmber());
    add(DraggablesEmber()..y = 350);
  }
}

// Note: this component does not consider the possibility of multiple
// simultaneous drags with different pointerIds.
class DraggablesEmber extends Ember with Draggables {
  @override
  bool debugMode = true;

  DraggablesEmber({Vector2? position})
      : super(
