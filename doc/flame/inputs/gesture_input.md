...
## Tappable, Draggables and Hoverables components

Any component derived from `Component` (most components) can add the `Tappable`, the
`Draggables`, and/or the `Hoverables` mixins to handle taps, drags and hovers on the component.
...
### Draggables components

Just like with `Tappable`, Flame offers a mixin for `Draggables`.

By adding the `HasDraggables` mixin to your game, and by using the mixin `Draggables` on
...
The default implementation provided by `Draggables` will already check:
...
class DraggablesComponent extends PositionComponent with Draggables {
...
class MyGame extends FlameGame with HasDraggables {
  MyGame() {
    add(DraggablesComponent());
  }
}
```

To recognize whether a `Draggables` added to the game handled an event, the `handled` field can be
...
```dart
class MyComponent extends PositionComponent with Draggables {
 @override
 bool onDragStart(DragStartInfo info) {
   info.handled = true;
   return true;
...
### Hoverables components

Just like the others, this mixin allows for easy wiring of your component to listen to hover states
and events.

By adding the `HasHoverables` mixin to your base game, and by using the mixin `Hoverables` on
...
To recognize whether a `Hoverables` added to the game handled an event, the `handled` field can be
set to true in the event can be checked in the corresponding method in the game class, or further
down the chain if you let the event continue to propagate.
...
```dart
class MyComponent extends PositionComponent with Hoverables {
  @override
  bool onHoverEnter(PointerHoverInfo info) {
    info.handled = true;
    return true;
