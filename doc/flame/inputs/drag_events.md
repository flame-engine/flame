...
1. Add the `HasDraggablesComponents` mixin to your main game class:

    ```dart
    class MyGame extends FlameGame with HasDraggablesComponents {
     // ...
    }
    ```
...
use the `DragCallbacks` mixin, and the "old-style" components that use the `Draggables` mixin. In
effect, every drag event will be propagated twice through the system: first trying to reach the
components with `DragCallbacks` mixin, and then components with `Draggables`.

```dart
class MyGame extends FlameGame with HasDraggablesComponents, HasDraggablesBridge {
  // ...
}
```

The purpose of this mixin is to ease the transition from the old event delivery system to the
new one. With this mixin, you can transition your `Draggables` components into using `DragCallbacks`
